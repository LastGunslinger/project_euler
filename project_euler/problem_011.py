...
def solve():
	...
	for row in range(0, 20):
		for column in range(0, 20):
			products.append(horizontal(grid, row, column))
...
def horizontal(grid, row, col):
	if col <= 16 :
		return product(grid[row][col:col+4])
	else:
		return 0

def vertical(grid, row, col):
	if row <= 16 :
		return product([ grid[row][col], grid[row+1][col], grid[row+2][col], grid[row+3][col] ])
	else:
		return 0

def diagonal_down(grid, row, col):
	if row <= 16 and col <= 16:
		return product([ grid[row][col], grid[row+1][col+1], grid[row+2][col+2], grid[row+3][col+3] ])
	else:
		return 0

def diagonal_up(grid, row, col):
	if row >= 3 and 3 <= col <= 16:
		return product([ grid[row][col], grid[row-1][col+1], grid[row-2][col+2], grid[row-3][col+3] ])
	else:
		return 0

def product(arg_list):
	if 0 in arg_list:
		return 0
	result = 1
	for x in arg_list:
		result *= x
	print(f'{arg_list} - {result}')
	return result
